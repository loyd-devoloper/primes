...
class Archived extends Component implements HasForms, HasTable
{
    ...
    public function table(Table $table): Table
    {
        ...
        ->query(LeaveEmployeeRequest::query()
                                            ->with(['employeeInfo'])
                                            ->where(function($q){
                                                $q->where('location', 'Records',)->where('type_of_process', 'ELECTRONIC SIGNATURE');
                                            })->orWhere('archived',1)->orderByDesc('id'))
            ->columns([
                ...
                TextColumn::make('type_of_process')->toggleable(true),
    ...
    {
        return view('livewire.leave.records.archived');
    }
}
