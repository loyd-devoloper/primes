...
@endassets
<div wire:ignore x-data="skillDisplay(@entangle('attributesData'))">
    <div class="grid grid-cols-1 gap-4  overflow-y-auto max-h-[80dvh] px-2 sm:px-5 xl:px-10"
        >
            ...
            <x-filament::section>
                <x-slot name="heading">
                    <div class="flex justify-between" wire:ignore>
                        EDIT TEMPLATE
                        <button wire:ignore.self x-on:click='generateMe'
                            class="flex items-center gap-2 shadow-inner bg-red-600 text-white py-2 px-5 rounded-md text-xs">
            ...
            @if (!!$activeTemplate)
                @if (!!$activeTemplateData->front)
                    <img id="imageFront"
                        ...
                        alt="">
                    <p class="absolute z-[99] hidden text-left break-words" id='firstname_f'>
                        {{ !!$userData->userInfo ? $userData->userInfo?->fname : '' }}
                    </p>
                    <p class="absolute   z-[99] hidden text-left  break-words" id='lastname_f'>
                        {{ !!$userData->userInfo ? $userData->userInfo?->lname : '' }}
                    </p>
                    <p class="absolute   z-[99] hidden text-center  break-words" id='middlename_f'>
                        {{ strtoupper(substr($userData->userInfo?->mname, 0, 1)) . '.' }}</p>
                    <p class="absolute   z-[99] hidden text-left  break-words" id='fistname_middlename_f'>
                        @if (!!$userData->userInfo?->mname)
                            {{ $userData->userInfo?->fname . ' ' . strtoupper(substr($userData->userInfo?->mname, 0, 1)) . '.' }}
                        @else
                            {{ !!!!$userData->userInfo ? $userData->userInfo?->fname : null }}
                        @endif
                    </p>
                    <p class="absolute   z-[99] hidden text-left  break-words" id='region_f'>Region IV-A
                        CALABARZON</p>
                    <p class="absolute   z-[99] hidden text-left  break-words" id='employee_id_f'>EMPLOYEE
                        NO. <span
                            class="opacity-70">{{ !!$userData->employee_id ? \Illuminate\Support\Facades\Crypt::decryptString($userData?->employee_id) : null }}</span>
                    </p>
                    <p class=" absolute   z-[99]  text-center h-fit w-fit  break-words hidden " id='office_name_f'>
                        {{ $userData?->workExperiencefirst?->position_title }}</p>
                    <p class=" absolute   z-[99]  text-left h-fit w-fit  break-words hidden " id='position_f'>
                        {{ $userData?->user_fd_code?->division_name }}

                    </p>
                    <img class="absolute  w-[10rem] hidden " id='profile_f'
                        src="{{ asset($userData?->profile ? 'storage/' . $userData->profile : 'assets/melvin.png') }}"
                        alt="">
                @endif
            @endif
        </div>
        <div class="relative   bg-green-500 ">
            @if (!!$activeTemplate)
                @if (!!$activeTemplateData->back)
                    ...
                    @if (!!$userData->userInfo?->mname)
                        <span class="absolute    z-[99] hidden text-center  break-words"
                            id='fullname_b'>{{ !!$userData?->userInfo ? $userData?->userInfo?->fname . ' ' . strtoupper(substr($userData->userInfo?->mname, 0, 1)) . '.' . ' ' . $userData?->userInfo?->lname : null }}</span>
                    @else
                        <span class="absolute    z-[99] hidden text-center  break-words"
                            id='fullname_b'>{{ !!$userData->userInfo ? $userData?->userInfo?->fname . ' ' . $userData?->userInfo?->lname : null }}</span>
                    @endif
                    <span class="absolute   z-[99] text-center hidden leading-none  break-words"
                        id='emergency_b'>{{ $userData?->userInfo?->contact_person_name }} <br>
                        {{ !!$userData?->userInfo ? \Illuminate\Support\Facades\Crypt::decryptString($userData?->userInfo?->contact_person_number) : null }}</span>
                    <img src="{{ $base64Image }}" id="qr_code_b" class="absolute hidden w-[10rem]" alt="">
                    {{-- <div id="qr_code_b" class="absolute hidden w-[10rem]">
                            {!! QrCode::size(AUTO)->generate('https://www.example.com') !!}
                        </div> --}}
                    <img src="{{ asset(!!$userData->otherInfo?->e_sig ? 'storage/' . $userData?->otherInfo?->e_sig : 'assets/SG1.png') }}"
                        id="e_sig_b" class="absolute hidden w-[10rem] " alt="" title="employee e_sig">
        ...
        Alpine.data('skillDisplay', (datas) => ({
            ...
            htmlTag(tagId, v, vfontSize, vfontWeight, vPosition, vfontColor, vWidth, vHeight, resizableBool =
                ...
                if (!!fname) {
                    ...
                    if (tagId !== 'lastname_f' && tagId !== 'fistname_middlename_f' && tagId !== 'position_f') {
            ...
            async init() {
                ...
                this.template_attribute_data = x;
