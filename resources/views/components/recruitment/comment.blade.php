        ...
        <div class="text-xs bg-gray-100 dark:bg-bgDark p-2 rounded-md flex justify-between">
            <div class="flex gap-2">
                ...
                <div class="grid dark:text-white ">
                    ...
                    <p class="text-gray-500">{{\Carbon\Carbon::parse($comment->created_at)->format('F d, Y h:i:s A')}}</p>
                    <p class="px-2 break-all whitespace-break-spaces break-words ">{{ $comment->comment }}</p>
                </div>
            ...
            @if (Auth::user()->id_number == $comment->id_number)
                ...
                <button type="button" class="text-red-500"
                        wire:click="deleteComment({{ $comment->id }},{{ $id }})">
