<!DOCTYPE html>
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" x-cloak x-data="{ darkMode: localStorage.getItem('darkMode') == 'true' }"
...
<head>
    ...
    <title>{{ $title ?? 'Prime System' }}</title>

    @filamentStyles
    @turnstileScripts()

    @vite(['resources/css/app.css', 'resources/js/app.js','resources/scss/christmas.scss'])
    @livewireStyles
    <style>
        [x-cloak] {
