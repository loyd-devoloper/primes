        ...
        <div class="space-y-4">
           {{-- @foreach ($getRecord()?->comments as $comment)
            ...
            <div class="bg-white dark:bg-bgDark p-4 rounded-lg shadow">
                ...
                <div class="text-gray-700 dark:text-gray-500">{!! str($comment->comment)->sanitizeHtml() !!}

                </div>
                <div class="flex items-center gap-2 mt-2">

                    <button type="button"  class="text-green-500 hover:text-green-600">Edit</button>
                    <span class="dark:text-gray-400">&#8226;</span>

                    <button type="button" class="text-red-500 hover:text-red-600" wire:click="deleteComment({{ $comment->id }})">Delete</button>
                    {{ ($this->slideOverDeleteCommentAction)(['id'=>1]) }}
